...
""" Erklärung Backtest """
"""
path_predictions: absoluter pfad zu dem csv / den csv files, falls gesplitted (im Falle BR Modell bspw.)
raw_data:         kompletten Rohdaten als DF (so wie Ihr die erstellt und einlest einfach)
staleness_filter: Wert für den Filter
model_name:       Ist optional, nur damit man bei der Auswertung von mehreren sieht welche gerade betrachtet werden
model_type:       Relevant für das Einlesen, da bei mir die predictions der universellen Modelle (LR, RF_univ) je in einem file
                  sind und die der individuellen (BR, RF_ind) in mehreren ---> für Euch jetzt unwichtig wohl und kann auskommentiert werden
modell:           Muss entsprechend als "classification" oder "regression" angegeben werden, damit der Backtest richtig auswertet
                  ---> bei "classification" Modellen hat man ja Wsken und bei den anderen absolut Werte, damit dann nur die 
                  über 50% als top gewertet werden
tc:               Wert für die Transaktionskosten

Wiedergabewerte:
kpi_df:           DF der KPI aus Tagesebene
kpi_trade_df:     DF der KPI auf Tradeebene
daily_rets:       Tageserenditen vor TC als DF
daily_rets:       Tagesrenditen nach TC als DF
"""


def kpi_backtest(path_predictions, raw_data, staleness_filter, model_name, model_type='universal', modell='classification', tc=0.002):
